...
# simulated genotypes outfiles names
simulated_genotypes_file_training = args.out + '.train.gz'
simulated_genotypes_file_testing = args.out + '.test.gz'
# population outfiles names
pops_file_training = args.out + '_pops.train.gz'
pops_file_testing = args.out + '_pops.test.gz'
...
while transposed_file_exist is False:
	try:
		...
		# open output files (training and testing datasets)
		with gzopen(simulated_genotypes_file_training, "wb") as out_training, \
			gzopen(simulated_genotypes_file_testing, "wb") as out_testing, \
				gzopen(pops_file_training, "wb") as out_pops_training, \
					gzopen(pops_file_testing, "wb") as out_pops_testing:
			...
			# print number of loci only in training, in testing it's assumed to be the same
			out_training.write(f"{num_snps}\n".encode('utf-8'))
			# print also number of pops (= number of output neurons) and num. of simulated offspring
			num_pops = len(args.pops)
			out_training.write(f"{num_pops}\n".encode('utf-8'))
			out_training.write(f"{args.train}\n".encode('utf-8'))
